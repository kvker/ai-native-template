---
name: an-init
description: 将现有工程转换为 AI Native 项目结构。当用户想要：1) 将现有项目转换为 AI Native 格式 2) 为已有代码生成 background/convention/workspace 文档 3) 执行 /an-init 命令 4) 让 AI 理解并接管现有项目 时触发此技能。
---
...
当用户描述功能开发意图（如"我要做一个XXX"、"添加XXX功能"、"需要XXX"）时，
先询问用户是否使用 /an-feature 标准工作流推进，提供以下选项：
...
> 根据识别到的技术栈调整 `paths` 中的文件后缀，只保留项目实际使用的语言。
...
3. 代码风格规范已写入 .claude/rules/，编辑代码时自动生效
```

...
| 信号 | 范围 | 跳过的阶段 |
|------|------|-----------|
| 明确的小 bug、配置修改、一两行改动 | 快速修复 | 跳过 requirements/design/tech-spec，直接 implementation，仍然需要撰写对应的文档，跳过的步骤则不撰写 |
| 有明确方案的改动（"按 XXX 方式实现"） | 标准开发 | 跳过 requirements/design，从 tech-spec 开始 |
| 新功能、涉及架构决策、用户只描述了想法 | 完整流程 | 不跳过任何阶段 |
...
**检查点**：向用户展示设计方案，确认技术选型和架构决策，然后询问进入下一阶段。
...
| 序号 | 变更项 | 文件路径 | 变更类型 | 说明 |
|------|--------|---------|---------|------|
| 1 | ... | target/apps/game/src/... | 新增/修改/删除 | ... |
